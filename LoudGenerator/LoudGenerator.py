...
def generate_cameras(num_cameras, max_entries=5, max_time_active=20, min_time_active=5, last_entry_time=50):
    cameras = []
    for _ in range(num_cameras):
        ...
        for _ in range(random.randint(1, max_entries)):
            entry_time = random.uniform(0, last_entry_time)
            exit_time = entry_time + random.uniform(min_time_active, max_time_active)
            entries.append((entry_time, exit_time))
        cameras.append({'fps': fps, 'period': period, 'entries': entries})
    return cameras

def simulate_frames(cameras, simulation_duration=60):
    events = []
    last_exit_time = 0
